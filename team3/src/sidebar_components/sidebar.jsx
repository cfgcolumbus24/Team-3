...
function Sidebar({ isAuthenticated }) {
  ...
  return (
    <div className="w-64 flex-shrink-0 bg-gray-800 h-screen transition-all duration-300 ease-in-out">
      ...
      {isModalOpen && (
        <div id="deleteModal" className="modal fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="modal-content bg-white p-4 rounded">
            <h4 className="text-lg font-bold">Confirm Deletion</h4>
            <p>Are you sure you want to delete this instructor?</p>
            <div className="mt-4">
              <button onClick={confirmDelete} className="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded mr-2">
                Yes
              </button>
              <button onClick={cancelDelete} className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-1 px-2 rounded">
                No
              </button>
            </div>
          </div>
